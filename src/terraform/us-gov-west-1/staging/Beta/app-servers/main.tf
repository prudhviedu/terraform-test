...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.mel-beta-app-servers"
...
#Beta App Servers (4) 902 & 903 stated not in use.
module "Vaausappmel900_Module" {
  ...
  tags                   = "${merge(
                              "${map("Name", "Vaausappmel900"
                              )}",
                              "${local.tags}"
                          )}"
 }
 module "Vaausappmel901_Module" {
  source = "../../../../modules/mel-server"
  env                    = "${var.env}"
  vpc                    = "${var.vpc}"
  instance_role          = "${local.instance_role}"
  ...
  subnet_id              = "${local.subnet_id}"
  tags                   = "${merge(
                              "${map("Name", "Vaausappmel901"
                              )}",
                              "${local.tags}"
                          )}"
 }
  module "Vaausappmel902_Module" {
  source = "../../../../modules/mel-server"
  env                    = "${var.env}"
  vpc                    = "${var.vpc}"
  instance_role          = "${local.instance_role}"
  ...
  subnet_id              = "${local.subnet_id}"
  tags                   = "${merge(
                              "${map("Name", "Vaausappmel902"
                              )}",
                              "${local.tags}"
                          )}"
 }
  module "Vaausappmel903_Module" {
  source = "../../../../modules/mel-server"
  env                    = "${var.env}"
  vpc                    = "${var.vpc}"
  instance_role          = "${local.instance_role}"
